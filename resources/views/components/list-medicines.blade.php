<section class="p-12 flex gap-3 overflow-x-hidden flex-col lg:flex-row">
    <div class="flex flex-col gap-5 justify-between w-full lg:w-1/4">
        <h1 class="text-5xl font-bold">List Obat</h1>
        <p class="text-xl text-wrap">Beberapa list obat yang kami tampilkan untuk anda</p>
        <div>
    ...
    <div class="flex gap-3 p-5 w-full lg:w-1/2 overflow-x-scroll">
                    ...
                    @if ($medicine->is_discount)
                        <div class="flex pr-12">
                            <p class="text-lg font-semibold text-red-500 line-through">Rp
                                {{ number_format($medicine->original_price, 0, ',', '.') }}
                            </p>

                            <p class="text-lg font-semibold text-green-600">Rp.
                                {{ number_format($medicine->discount_price, 0, ',', '.') }}
                            </p>
                        </div>
                    @else
                        <p class="text-lg font-semibold">Rp {{ number_format($medicine->price, 0, ',', '.') }}</p>
                    @endif
