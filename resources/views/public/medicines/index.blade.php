    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="text-center mb-12">
            ...
            <p class="text-lg text-gray-600 max-w-2xl mx-auto leading-relaxed">
                Temukan berbagai pilihan obat untuk kebutuhan kesehatan Anda. Cari dan temukan dengan mudah obat yang Anda butuhkan.
            </p>
        </div>
        ...
        <!-- Results Info -->
        @if(request()->has('search') || request()->has('category') || request()->has('unit'))
        <div class="mb-6 text-center">
            <p class="text-gray-600">
                Menampilkan 
                <span class="font-semibold text-gray-900">{{ $medicines->total() }}</span> 
                hasil
                @if(request('search'))
                    untuk "<span class="font-semibold text-red-600">{{ request('search') }}</span>"
                @endif
                @if(request('category'))
                    dalam kategori <span class="font-semibold text-red-600">{{ request('category') }}</span>
                @endif
                @if(request('unit'))
                    dengan satuan <span class="font-semibold text-red-600">{{ request('unit') }}</span>
                @endif
            </p>
        </div>
        @endif
        ...
        <!-- Quick Stats -->
        <div class="mt-12 bg-white rounded-2xl shadow-md border border-gray-100 p-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
                <div class="space-y-2">
                    <div class="text-2xl font-bold text-red-600">{{ $medicines->total() }}+</div>
                    <div class="text-gray-600">Jenis Obat Tersedia</div>
                </div>
                <div class="space-y-2">
                    <div class="text-2xl font-bold text-red-600">{{ $categories->count() }}+</div>
                    <div class="text-gray-600">Kategori Obat</div>
                </div>
                <div class="space-y-2">
                    <div class="text-2xl font-bold text-red-600">24/7</div>
                    <div class="text-gray-600">Ketersediaan</div>
                </div>
            </div>
        </div>
    </div>
